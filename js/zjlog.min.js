...
(function(){var APP_ID='FbYwxeLvurOiHNxLsomyocYR-gzGzoHsz';var APP_KEY='ib6XFRh25EnYXyVgTmurDD0f';var logFlag='100day';var url='https://zhoujie16.github.io/100days/';AV.init({appId:APP_ID,appKey:APP_KEY});var userClient=navigator.userAgent.toLowerCase();var tableName=localStorage.getItem(url);if(!tableName){tableName=logFlag+'_'+randomString(8);localStorage.setItem(url,tableName)}var AVObject=AV.Object.extend(tableName);window.zjLog=function(msg){var avObject=new AVObject();avObject.set('console',msg);avObject.set('userClient',userClient);avObject.save().then(function(avObject){},function(error){})};function randomString(len){len=len||32;var $chars='ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678';var maxPos=$chars.length;var pwd='';for(i=0;i<len;i+=1){pwd+=$chars.charAt(Math.floor(Math.random()*maxPos))}return pwd}function userBrowser(){var browserName=navigator.userAgent.toLowerCase();if(/msie/i.test(browserName)&&!/opera/.test(browserName)){return("IE")}else if(/firefox/i.test(browserName)){return("Firefox")}else if(/chrome/i.test(browserName)&&/webkit/i.test(browserName)&&/mozilla/i.test(browserName)){return("Chrome")}else if(/opera/i.test(browserName)){return("Opera")}else if(/webkit/i.test(browserName)&&!(/chrome/i.test(browserName)&&/webkit/i.test(browserName)&&/mozilla/i.test(browserName))){return("Safari")}else{return("unKnow")}}function getOSType(){if(/(Android)/i.test(navigator.userAgent)){return 'android'}else if(/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)){return 'iOS'}else{return 'unknow'}}function is_weixin(){var ua=navigator.userAgent.toLowerCase();if(ua.match(/MicroMessenger/i)=="micromessenger"){return true}else{return false}}}());
